...
**Step 1: Finding "bookkeeping columns"** (look at the first 15 column names, since those columns would most likely show up in the beginning):  
...
We will use the function **nearZeroVar()** from the **caret** package. **saveMetrics=FALSE** returns the list of indices of near zero variance predictors.  
...
Lastly, let's find out the rate of NAs in the remaining columns:  
...
The test set now has the same 52 predictors as the training set. The last step in cleaning the data will be to "factorise" outcome variable **classe** in the Train set:  
...
As expected, the out of sample error rates (a.k.a. test error rates) are higher than the in sample/train error rates above, and the accuracy rate is lower. However, the accuracy rate is still very high, and so is Kappa statistic (which indicates how closely the instances classified by the machine learning classifier matched the true results), and error rates for each class seem to be extremely low (see Statistics by Class in the output above):  
```{r echo=FALSE}
paste("Model Accuracy:", "", round(cmRF$overall["Accuracy"], 4))
paste("Kappa Statistic:", "", round(cmRF$overall["Kappa"], 4))
```

We can see that the reputation of Random Forest as one of the most accurate prediction algorithms is not unfounded.
